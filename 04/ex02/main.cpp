...
int	main(void)
{
	...
	//Создание объекта родительского класса Animal
	/*
	std::cout << std::endl << "########## Sozdanie obyekta roditelskogo klassa ##########" << std::endl;
	Animal	*emptyAnimal = new Animal();
	std::cout << "Type of emptyAnimal" << " is «" << emptyAnimal->getType() << "»" << std::endl;
	emptyAnimal->makeSound();
	delete emptyAnimal;
	*/
